...
f_cur_new = cell(J+1-args.J_min, 2*N-1);  
for j = args.J_min:J 
   for en = 1: 2*N-1 
   f_cur_new{j- args.J_min+1, en} = reshape(f_cur{j- args.J_min+1}(en,:),  args.L, 2* args.L-1);
   end
end   
figure
ssht_plot_mollweide(f_cur_new{1, 1},  args.L, 'Mode', 1);
