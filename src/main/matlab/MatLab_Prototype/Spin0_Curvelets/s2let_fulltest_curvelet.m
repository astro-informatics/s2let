...
% Main parameters
L = 16;
N = L; % For curvelets: N=L always holds. 
B = 2;  %4
Spin = 0; 
J_min = 0; %1
J = s2let_jmax(L, B);
...
disp('ana_lm2lmn : Perform scalar harmonic-to-Wigner transform (lm2lmn) with default parameters')
[f_cur_lmn, f_scal_lm]= s2let_transform_curvelet_analysis_lm2lmn(flm_gen_cur, cur_lm, scal_l,  'Upsample', true);
disp('syn_lmn2lm : Perform inverse transform (lmn2lm) with default parameters')
flm_rec_lmn2lm = s2let_transform_curvelet_synthesis_lmn2lm(f_cur_lmn, f_scal_lm, cur_lm, scal_l, 'Upsample', true);
default = max(abs(flm_gen_cur-flm_rec_lmn2lm))


disp('ana_lm2lmn : Perform (spin) harmonic-to-Wigner transform (lm2lmn) with default parameters')
[f_cur_lmn, f_scal_lm]=  s2let_transform_curvelet_analysis_lm2lmn(flm_gen_cur,  cur_lm, scal_l, ...
                                                                 'Spin', Spin,'Upsample', true);
disp('syn_lmn2lm : Perform inverse transform (lmn2lm) with default parameters')
flm_rec_spin_lmn2lm =s2let_transform_curvelet_synthesis_lmn2lm(f_cur_lmn, f_scal_lm, cur_lm, scal_l,...
                                                                 'Spin', Spin, 'Upsample', true);
default = max(abs(flm_gen_cur- flm_rec_spin_lmn2lm))
...
disp('syn_cur2lm:  Perform inverse transform (cur2lm) with default parameters')
...
disp('ana_lm2cur: Perform (spin) harmonic-to-curvelet transform (lm2cur) with default parameters')
[f_cur, f_scal] = s2let_transform_curvelet_analysis_lm2cur(flm_gen_cur, 'Spin', Spin, 'Upsample', true);
disp('syn_cur2lm:  Perform inverse transform (cur2lm) with default parameters')
flm_rec_spin_lm2cur = s2let_transform_curvelet_synthesis_cur2lm(f_cur, f_scal,'Spin', Spin, 'Upsample', true);
...
[f_cur, f_scal] = s2let_transform_curvelet_analysis_lm2cur(flm_gen_cur, ...
                                                           'B', B, 'L', L, 'J_min', J_min,...
                                                           'Spin', Spin, 'Upsample',true);
...
disp('-------------')
% Test: signal reconstruction in pixel space: 
disp('ana_px2cur: Perform scalar curvelet transform (px2cur) with default parameters')
[f_cur, f_scal] = s2let_transform_curvelet_analysis_px2cur(f_gen_cur,  'Upsample', true);
disp('syn_cur2px:  Perform inverse transform (cur2px) with default parameters')
f_rec_px2cur = s2let_transform_curvelet_synthesis_cur2px(f_cur, f_scal, 'Upsample', true);
default = max(max(abs(f_gen_cur-f_rec_px2cur)))
...
f_rec_spin_px2cur = s2let_transform_curvelet_synthesis_cur2px(f_cur, f_scal, ...
                                                    'Spin', Spin, 'Upsample', true);
default = max(max(abs(f_spin_gen_cur -f_rec_spin_px2cur)))
