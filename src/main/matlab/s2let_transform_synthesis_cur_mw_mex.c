...
/**
 * MATLAB interface: s2let_transform_synthesis_cur_mw_mex.c
 * This function for internal use only.
 * Compute spin directional curvelet transform (synthesis)
 * with output in pixel space.
 *
 * Usage:
 *   f = ...
 *        s2let_transform_synthesis_cur_mw_mex(f_cur, f_scal, B, L, J_min, N, spin, reality, upsample,
                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  if(nrhs!=12) {
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:nrhs",
          "Require twelve inputs.");
  }
  if(nlhs!=1) {
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidOutput:nlhs",
          "Require two outputs.");
  }
  ...
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:reality",
  ...
  iin = 8;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:upsample",
  ...
  if( !mxIsChar(prhs[iin]) ) {
      mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:samplingSchemeChar",
                        "Sampling scheme must be string.");
  }
  int len = (mxGetM(prhs[iin]) * mxGetN(prhs[iin])) + 1;
  if (len >= S2LET_STRING_LEN)
      mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:samplingSchemeTooLong",
  ...
  if (strcmp(sampling_str, S2LET_SAMPLING_MW_STR) == 0)
      ...
      mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:samplingScheme",
                        "Invalid sampling scheme.");

  // Parse normalization flag
  iin = 9;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:spinlowered",
  ...
  if( !mxIsDouble(prhs[iin]) ||
    ...
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:spinloweredfrom",
          "SpinLoweredFrom must be integer.");
  }
  original_spin = (int)mxGetScalar(prhs[iin]);

  // Parse input curvelets f_cur
  iin = 0;
  f_m = mxGetM(prhs[iin]);
  f_n = mxGetN(prhs[iin]);
  f_cur_real = mxGetPr(prhs[iin]);
  if(reality){
    f_cur_r = (double*)malloc( f_m*f_n * sizeof(double));
    for(j=0; j<f_m*f_n; j++)
      f_cur_r[ j ] = f_cur_real[ j ];
  }else{
    f_cur_imag = mxGetPi(prhs[iin]);
    f_cur = (complex double*)malloc( f_m*f_n * sizeof(complex double));
    for(j=0; j<f_n*f_m; j++)
      f_cur[ j ] = f_cur_real[ j ]
          + I * f_cur_imag[ j ] ;
  }
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:curveletParameter",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:LbandLimit",
          "Harmonic band-limit L must be integer.");
  }
  L = (int)mxGetScalar(prhs[iin]);

  if (mxGetScalar(prhs[iin]) > (double)L || L <= 0)
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_synthesis_cur_mw_mex:InvalidInput:Jmin",
  ...
  if(reality){
      if (sampling_scheme == S2LET_SAMPLING_MW_SS)
          s2let_allocate_mwss_real(&f_r, L);
      else
          s2let_allocate_mw_real(&f_r, L);
      s2let_synthesis_cur2px_real(f_r, f_cur_r, f_scal_r, &parameters);
  }else{
      if (sampling_scheme == S2LET_SAMPLING_MW_SS)
          s2let_allocate_mwss(&f, L);
      else
          s2let_allocate_mw(&f, L);
      s2let_synthesis_cur2px(f, f_cur, f_scal, &parameters);
  }
