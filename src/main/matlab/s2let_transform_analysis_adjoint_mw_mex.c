...
/**
 * MATLAB interface: s2let_transform_analysis_adjoint.
 * This function for internal use only.
 * Compute spin directional wavelet transform (analysis_adjoint)
 * with output in pixel space.
 *
 * Usage:
 *   f = ...
 *        s2let_transform_analysis_adjoint_mw_mex(f_wav, f_scal, B, L, J_min, N, spin, reality, upsample,
                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  if(nrhs!=12) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:nrhs",
          "Require twelve inputs.");
  }
  if(nlhs!=1) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidOutput:nlhs",
          "Require two outputs.");
  }
  ...
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:reality",
  ...
  iin = 8;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:upsample",
  ...
  if( !mxIsChar(prhs[iin]) ) {
      mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:samplingSchemeChar",
                        "Sampling scheme must be string.");
  }
  int len = (mxGetM(prhs[iin]) * mxGetN(prhs[iin])) + 1;
  if (len >= S2LET_STRING_LEN)
      mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:samplingSchemeTooLong",
  ...
  if (strcmp(sampling_str, S2LET_SAMPLING_MW_STR) == 0)
      ...
      mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:samplingScheme",
                        "Invalid sampling scheme.");

  // Parse normalization flag
  iin = 9;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:spinlowered",
  ...
  if( !mxIsDouble(prhs[iin]) ||
    ...
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:spinloweredfrom",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:waveletParameter",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:LbandLimit",
          "Harmonic band-limit L must be integer.");
  }
  L = (int)mxGetScalar(prhs[iin]);

  if (mxGetScalar(prhs[iin]) > (double)L || L <= 0)
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_adjoint_mw_mex:InvalidInput:Jmin",
  ...
  if(reality){
      ...
      s2let_analysis_adjoint_wav2px_real(f_r, f_wav_r, f_scal_r, &parameters);
  }else{
      if (sampling_scheme == S2LET_SAMPLING_MW_SS)
          s2let_allocate_mwss(&f, L);
      else
          s2let_allocate_mw(&f, L);
      s2let_analysis_adjoint_wav2px(f, f_wav, f_scal, &parameters);
  }
