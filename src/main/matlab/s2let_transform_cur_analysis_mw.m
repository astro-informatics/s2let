function [f_cur, f_scal] = s2let_transform_cur_analysis_mw(f, varargin)

% s2let_transform_cur_analysis_mw
% Compute curvelet transform, output in pixel space.
%
% Default usage :
%
%   [f_cur, f_scal] = s2let_transform_cur_analysis_mw(f, <options>)
%
...
[f_cur_vec, f_scal_vec] = s2let_transform_cur_analysis_mw_mex(f_vec, args.B, args.L, args.J_min, ...
                                                          args.N, args.Spin, ...
...
for j = args.J_min:J
  ...
  for t = 0:band_limit-1
      for p = 0:2*band_limit-2
          ...
          temp(t+1,p+1) = f_cur_vec(1,ind);
      end
  end
  f_cur{j+1-args.J_min} = temp;
