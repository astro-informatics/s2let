...
/**
 * MATLAB interface: s2let_transform_analysis_lm2cur_mex.
 * This function for internal use only.
 * Compute spin directional curvelet transform (analysis)
 * with input in harmonic space and output in pixel space.
 *
 * Usage:
 *   [f_cur, f_scal] = ...
 *        s2let_transform_analysis_lm2cur_mex(flm, B, L, J_min, N, spin, reality, upsample,
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  double *f_cur_real, *f_scal_real, *flm_real;
  double *f_cur_imag, *f_scal_imag, *flm_imag;
  complex double *f_cur = NULL, *f_scal = NULL, *flm= NULL;
  double *f_cur_r = NULL, *f_scal_r = NULL;
  int iin = 0, iout = 0;
  // Check number of arguments
  if(nrhs!=11) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:nrhs",
          "Require eleven inputs.");
  }
  if(nlhs!=2) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidOutput:nlhs",
          "Require two outputs.");
  }
  ...
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:reality",
  ...
  iin = 7;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:upsample",
  ...
  if( !mxIsChar(prhs[iin]) ) {
      mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:samplingSchemeChar",
                        "Sampling scheme must be string.");
  }
  int len = (mxGetM(prhs[iin]) * mxGetN(prhs[iin])) + 1;
  if (len >= S2LET_STRING_LEN)
      mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:samplingSchemeTooLong",
  ...
  if (strcmp(sampling_str, S2LET_SAMPLING_MW_STR) == 0)
      ...
      mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:samplingScheme",
                        "Invalid sampling scheme.");

  // Parse normalization flag
  iin = 8;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:spinlowered",
  ...
  if( !mxIsDouble(prhs[iin]) ||
    ...
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:spinloweredfrom",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:curveletParameter",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:LbandLimit",
          "Harmonic band-limit L must be integer.");
  }
  L = (int)mxGetScalar(prhs[iin]);

  if (mxGetScalar(prhs[iin]) > (double)L || L <= 0)
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:Jmin",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:NbandLimit",
  ...
  if( !mxIsDouble(prhs[iin]) ||
      mxIsComplex(prhs[iin]) ||
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:spin",
          "spin must be integer.");
  }
  spin = (int)mxGetScalar(prhs[iin]);

  if (reality && spin)
    mexErrMsgIdAndTxt("s2let_transform_analysis_lm2cur_mex:InvalidInput:realspin",
  ...
  if(reality){
      s2let_allocate_f_cur_real(&f_cur_r, &f_scal_r, &parameters);
      s2let_analysis_lm2cur_real(f_cur_r, f_scal_r, flm, &parameters);
  }else{
  ...
  if(upsample){
    so3_parameters.L = L;
    cursize = (J+1-J_min) * so3_sampling_f_size(&so3_parameters);
    ...
    for (j = J_min; j <= J; j++){
        ...
        cursize += so3_sampling_f_size(&so3_parameters);
     }
