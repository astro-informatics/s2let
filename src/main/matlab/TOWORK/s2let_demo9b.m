% s2let_demo9b - curvelets (modified from demo1)
...
[f_cur, f_scal] = s2let_transform_cur_analysis_mw(f, 'B', B, 'J_min', J_min, 'Reality', true, 'Upsample', true);
...
for j = J_min:J
   ...
   ssht_plot_mollweide(f_cur{j-J_min+1}, L);
   campos([0 0 -1]); camup([0 1 0]); zoom(zoomfactor)
v = caxis;
temp = max(abs(v));
caxis([-temp temp])
   %title(['Curvelet scale : ',int2str(j)-J_min+1])
end


% Perform decomposition
[f_cur, f_scal] = s2let_transform_cur_analysis_mw(f, 'B', B, 'J_min', J_min, 'Reality', true);
...
for j = J_min:J
   ...
   ssht_plot_mollweide(f_cur{j-J_min+1}, bl);
   campos([0 0 -1]); camup([0 1 0]); zoom(zoomfactor)
v = caxis;
temp = max(abs(v));
caxis([-temp temp])
