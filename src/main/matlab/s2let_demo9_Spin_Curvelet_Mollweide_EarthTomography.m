...
% -----------------------------------------------------------
% Remark:
% Since Earth Tomographic data are real
% Use mode 1 setting for ssht_plot_mollweide 
...
ny = 6; %16  % ns - 1 + rem(2+J-J_min+1 , ns) ;
nx = 4;  %4 % ns;
...
% ============
% Use directional wavelets for analysis 
% (To compare with the case using curvelets)
% ============
%{
...
for j = J_min:J
	for en = 1:N
        ...
        if ind <= maxfigs
            ...
            temp = max(abs(v));
            caxis([-temp temp])
            title(['Wavelet scale j=',int2str(j)-J_min+1,', n=',int2str(en)],'FontSize',8)
        end
	end
end
colormap(jet)
