...
% ============
[f_wav, f_scal_mw] = s2let_transform_analysis_mw(f_bandlimit_mw, 'B', B, 'J_min', J_min, ...
                                                 'N', N, 'Reality',reality,...
...
caxis([-temp temp])
subplot(ny, nx, 2);
ssht_plot_mollweide(f_scal_mw, L);
campos([0 0 -1]); camup([0 1 0]); zoom(zoomfactor)
v = caxis;
temp = max(abs(v));
caxis([-temp temp])
...
% Plot histogram: 
reshape_f_wav = reshape(abs(f_wav{J-J_min+1}), prod(size(f_wav{J-J_min+1})),1); 
figure;
% h= histogram(reshape_f_wav) %or in short histogram(f_wav{J-J_min+1}(:))
% nbins = h.NumBins
nbins =L 
h1= histogram(reshape_f_wav, nbins)
h1.FaceColor = [0 0 1];
...
reshape_f_cur = reshape(abs(f_cur_new{J-J_min+1}), prod(size(f_cur_new{J-J_min+1})),1); 
h2= histogram(reshape_f_cur, nbins)
h2.FaceColor = [0 1 1];                                  
