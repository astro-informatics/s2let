function [flm_cur_syn, flm_scal_syn] = s2let_transform_synthesis_lmn2lm(f_cur_lmn, f_scal_lm, cur_lm, scal_l, flm_init, varargin)
...
% Default usage :
%
%   [flm_cur_syn, f_scal_syn] = s2let_transform_synthesis_lmn2lm(f_cur_lmn, f_scal_lm, cur_lm, scal_l,flm_init, <options>)
%
...
% flm_init contains the spherical harmonic coefficients of the initial
% signals. 
%
% Option :
...
p.parse(f_cur_lmn, f_scal_lm, cur_lm, scal_l, flm_init, varargin{:});
args = p.Results;
...
for el = 0:args.L-1,
  ...
  for m = -el:el,
   ...
   flm_scal_syn(lm_ind) =  flm_scal_syn(lm_ind)+ flm_init(lm_ind)* phi;
  end
end
