...
void s2let_transform_axisym_wav_synthesis_mw(
    ...
    for(j = J_min; j <= J; j++){
        ...
        ssht_core_mw_forward_sov_conv_sym(f_wav_lm + offset_lm, f_wav_norm, L, spin, dl_method, verbosity);
        offset_lm += L * L;
        offset += L * (2 * L - 1);
    }
    free(f_wav_norm);
...
void s2let_transform_axisym_wav_synthesis_adjoint_mw(
    ...
    for (j = J_min; j <= J; ++j)
    {
        ...
        ssht_adjoint_mw_forward_sov_sym(f_wav + offset, f_wav_lm + offset_lm, L, spin, dl_method, verbosity);
 ...
 * Perform multiresolution wavelet transform adjoint in real space (from scratch, gives wavelet maps).
 * Sampling scheme : MW sampling.
 *
 * \param[out]  f Input function (MW sampling)
 * \param[in]  f_wav Array of wavelets maps, MW sampling.
...
void s2let_transform_axisym_wav_analysis_adjoint_mw_multires(
    ...
    bandlimit = MIN(s2let_bandlimit(J_min-1, parameters), L);
    ssht_adjoint_mw_inverse_sov_sym(f_scal_lm, f_scal, bandlimit, spin, dl_method, verbosity);
    offset = 0;
    offset_lm = 0;
    for(j = J_min; j <= J; j++){
        int i;
...
void s2let_transform_axisym_wav_synthesis_mw_multires(
    ...
    for(j = J_min; j <= J; j++){
        ...
        ssht_core_mw_forward_sov_conv_sym(f_wav_lm + offset_lm, f_wav_norm, bandlimit, spin, dl_method, verbosity);
        offset_lm += bandlimit * bandlimit;
        offset += bandlimit * (2 * bandlimit - 1);
 ...
 * Perform multiresolution wavelet inverse transform adjoint in real space (from scratch, gives wavelet maps).
 * Sampling scheme : MW sampling.
 *
 * \param[out]  f_wav Array of wavelets maps, MW sampling.
 * \param[out]  f_scal Scaling function map, MW sampling.
...
void s2let_transform_axisym_wav_synthesis_adjoint_mw_multires(
    ...
    bandlimit = MIN(s2let_bandlimit(J_min-1, parameters), L);
    ssht_adjoint_mw_forward_sov_sym(f_scal, f_scal_lm, bandlimit, spin, dl_method, verbosity);
    offset = 0;
    offset_lm = 0;
    for(j = J_min; j <= J; j++){
        int i;
        ...
        bandlimit = MIN(s2let_bandlimit(j, parameters), L);
        ssht_adjoint_mw_forward_sov_sym(f_wav + offset, f_wav_lm + offset_lm, bandlimit, spin, dl_method, verbosity);
...
void s2let_transform_axisym_wav_synthesis_mw_real(
    ...
    for(j = J_min; j <= J; j++){
        ...
        ssht_core_mw_forward_sov_conv_sym_real(f_wav_lm + offset_lm, f_wav_norm, L, dl_method, verbosity);
        offset_lm += L * L;
        offset += L * (2 * L - 1);
    }
    free(f_wav_norm);
...
/*!
 * Perform wavelet inverse transform adjoint in real space (from scratch, gives wavelet maps).
 * Input function is real.
 * Sampling scheme : MW sampling.
 * Spherical wavelets : analysis in real space, MW sampling.
 *
...
void s2let_transform_axisym_wav_synthesis_adjoint_mw_real(
    ...
    for(j = J_min; j <= J; j++){
        ...
        ssht_adjoint_mw_forward_sov_sym_real(f_wav + offset, f_wav_lm + offset_lm, L, dl_method, verbosity);
...
void s2let_transform_axisym_wav_synthesis_mw_multires_real(
    ...
    for(j = J_min; j <= J; j++){
        ...
        ssht_core_mw_forward_sov_conv_sym_real(f_wav_lm + offset_lm, f_wav_norm, bandlimit, dl_method, verbosity);
        offset_lm += bandlimit * bandlimit;
        offset += bandlimit * (2 * bandlimit - 1);
