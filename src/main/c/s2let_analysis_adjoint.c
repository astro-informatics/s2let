 ...
 * \param[in]  f_wav_lmn Wavelet transform (Wigner coefficients of wavelet
 * contribution). \param[in]  f_scal_lm Wavelet transform (spherical harmonic
 * coefficients of scaling contribution). \param[in]  wav_lm Wavelet kernels. \param[in]
 * scal_l Scaling function kernels. \param[in]  parameters A fully populated parameters
 * object. The \link s2let_parameters_t::reality reality\endlink flag is ignored. Use
 * \link s2let_synthesis_lmn2lm_real \endlink instead for real signals. \retval none
 */
 ...
 * scal_l Scaling function kernels. \param[in]  parameters A fully populated parameters
 * object. The \link s2let_parameters_t::reality reality\endlink flag is ignored. Use
 * \link s2let_synthesis_lmn2lm \endlink instead for complex signals. \retval none
 */
...
void s2let_analysis_adjoint_wav2lm_manual(
  ...
  case S2LET_SAMPLING_MW:
    ssht_adjoint_mw_inverse_sov_sym(
        f_scal_lm, f_scal, bandlimit, 0, parameters.dl_method, verbosity);
    break;
  ...
  for (j = 0; j <= J; ++j) {
    ...
    so3_adjoint_inverse_direct(f_wav_lmn, f_wav + offset, &so3_parameters);
...
void s2let_analysis_adjoint_wav2lm(
  ...
  case S2LET_SAMPLING_MW:
    ssht_adjoint_mw_inverse_sov_sym(
        f_scal_lm, f_scal, bandlimit, 0, dl_method, verbosity);
    break;
  case S2LET_SAMPLING_MW_SS:
    ssht_adjoint_mw_inverse_sov_sym_ss(
        f_scal_lm, f_scal, bandlimit, 0, dl_method, verbosity);
  ...
  // {
  // case S2LET_SAMPLING_MW:
  //     ssht_core_mw_forward_sov_conv_sym(f_scal_lm, f_scal, bandlimit, 0, dl_method,
  //     verbosity); break;
  // case S2LET_SAMPLING_MW_SS:
  //     ssht_core_mw_forward_sov_conv_sym_ss(f_scal_lm, f_scal, bandlimit, 0,
  //     dl_method, verbosity); break;
  // default:
  //     S2LET_ERROR_GENERIC("Sampling scheme not supported.");
  // }
...
void s2let_analysis_adjoint_wav2lm_real(
  ...
  for (j = J_min; j <= J; ++j) {
    ...
    so3_adjoint_inverse_direct_real(
        f_wav_lmn + offset_lmn, f_wav + offset, &so3_parameters);
...
void s2let_analysis_adjoint_wav2px(
  ...
  case S2LET_SAMPLING_MW:
    ...
    break;
  case S2LET_SAMPLING_MW_SS:
    ssht_adjoint_mw_forward_sov_sym_ss(f, flm, L, spin, dl_method, verbosity);
    break;
  default:
    S2LET_ERROR_GENERIC("Sampling scheme not supported.");
  }
...
void s2let_analysis_adjoint_wav2px_real(
  ...
  case S2LET_SAMPLING_MW:
    ...
    break;
  case S2LET_SAMPLING_MW_SS:
    ssht_adjoint_mw_forward_sov_sym_ss_real(f, flm, L, dl_method, verbosity);
    break;
  default:
    S2LET_ERROR_GENERIC("Sampling scheme not supported.");
  }
