...
void s2let_fits_mw_write_map(char *filename, double *f, int L) {
  ...
  if (fits_create_tbl(
          fptr, BINARY_TBL, npix, tfields, ttype, tform, tunit, extname, &status))
    fprintf(
        stderr, "%s (%d): Could not create new binary table.\n", __FILE__, __LINE__);

  if (fits_write_key(fptr, TINT, "L", &L, "Resolution parameter", &status))
...
void s2let_fits_mwss_write_map(char *filename, double *f, int L) {
  ...
  int mwss = 1;
  if (fits_write_key(fptr, TINT, "MWSS", &mwss, "Sampling scheme parameter", &status))
