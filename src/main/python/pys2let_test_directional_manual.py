...
hybrid_scal_l, hybrid_wav_l, hybrid_scal_bandlimit, hybrid_wav_bandlimits, J, L_bounds = construct_hybrid_tiling(
    L, J_min, Bs, L_transitions)
...
for k in range(Bs.size):
    ...
    axs[k].set_xlim([1, L])
    axs[k].set_title(
        'Tiling %i' % (k + 1) + ' on %i' % Bs.size + ', with B=%.2f' % Bs[k] + ', Jmin=0, and defined at %i' % L_bounds[k] + '<el<%i' % L_bounds[k + 1])
...
res = verify_tiling(L, hybrid_scal_l, hybrid_wav_l,
                    hybrid_scal_bandlimit, hybrid_wav_bandlimits)
...
f_wav, f_scal = analysis_lm2wav_manualtiling(
    f_lm, L, N, spin, hybrid_scal_l, hybrid_wav_l, hybrid_scal_bandlimit, hybrid_wav_bandlimits)

# Uses synthesis to reconstruct the input alms.
print('Running synthesis_wav2lm')
f_lm_rec = synthesis_wav2lm_manualtiling(
    f_wav, f_scal, L, N, spin, hybrid_scal_l, hybrid_wav_l, hybrid_scal_bandlimit, hybrid_wav_bandlimits)
...
print('ACCURACY HARM:', np.max(f_mw_rec - f_mw),
      np.mean(f_mw_rec / f_mw), np.std(f_mw_rec / f_mw))
print('ACCURACY REAL:', np.max(f_lm_rec - f_lm),
      np.mean(f_lm_rec / f_lm), np.std(f_lm_rec / f_lm))
...
for j in range(0, J + 1):
    for n in range(0, N):
        ...
        print('plot id', j * N + n, 'j=', j, 'n=', n,
              'bounds=', offset, 'to', offset + nelem)
        # Make the plot!
